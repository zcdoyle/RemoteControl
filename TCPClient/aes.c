...
{
    int block_len = AES_KEY_LENGTH/8;
    return in_len%block_len == 0? in_len:in_len+block_len-in_len%block_len;
}
...
{
    AES_Init(key);      //初始化秘钥
    AES_Encrypt(str, str, len, (unsigned char*)IV);   //加密
}
...
{
    AES_Init(key);      //初始化秘钥
    AES_Decrypt(str, str, len, (unsigned char*)IV);
}
